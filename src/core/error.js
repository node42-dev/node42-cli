...
import { WWW_URL } from './constants.js';
...
export const N42ErrorCode = Object.freeze({
  ...
  // SMP
  SMP_NOT_FOUND:        { code: 12105, area: 'DISC', http: 502, message: 'SMP not found: {details}',                   retryable: false },
  SMP_ERROR:            { code: 12105, area: 'DISC', http: 502, message: 'SMP error: {details}',                       retryable: true  },

  // Input
  INVALID_INPUT:        { code: 7020,  area: 'APP',  http: 400, message: 'Invalid input: {details}',                   retryable: false },

  // File system
  DIR_NOT_FOUND:        { code: 7020,  area: 'APP',  http: 400, message: 'Directory not found: {details}',             retryable: false },
  FILE_NOT_FOUND:       { code: 7020,  area: 'APP',  http: 400, message: 'File not found: {details}',                  retryable: false },

  // Documents
  DOC_INVALID:          { code: 7021,  area: 'APP',  http: 400, message: 'Document is invalid: {details}',             retryable: false },
  DOC_NOT_FOUND:        { code: 7020,  area: 'APP',  http: 400, message: 'Document not found: {details}',              retryable: false },
...
export class N42Error extends Error {
  constructor(errorDef, variables = {}, { url = null } = {}) {
...
export function handleApiError(err) {
  ...
  const url  = code ? `${WWW_URL}/errors?code=${code}` : `${WWW_URL}/errors`;
  const link = `\u001B]8;;${url}\u0007View Details\u001B]8;;\u0007`;

 if (message) {
    console.error(`\r${c(C.BOLD, `Error: ${code}`)} ${c(C.BLUE, `[${link}]`)}\n\n${c(C.RED, err.message)}\n`);
  } else {
    console.error(`\r${c(C.BOLD, `Error: ${code}`)} ${c(C.BLUE, `[${url}]`)}\n\nFor details, see the documentation.\n`);
  }
}